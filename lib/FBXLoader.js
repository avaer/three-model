...
module.exports = function({THREE}) {
	...
	var THREEFBXLoader = function ( manager ) {
	...
	function getTexture( FBXTree, textureMap, id, connections ) {
		...
		if ( 'LayeredTexture' in FBXTree.Objects.subNodes && id in FBXTree.Objects.subNodes.LayeredTexture ) {
			...
			console.warn( 'THREEFBXLoader: layered textures are not supported in three.js. Discarding all but first layer.' );
		...
		if ( 'Deformer' in FBXTree.Objects.subNodes ) {
			...
			for ( var nodeID in DeformerNodes ) {
				...
				if ( deformerNode.attrType === 'Skin' ) {
					...
					if ( relationships.parents.length > 1 ) console.warn( 'THREEFBXLoader: skeleton attached to more than one geometry is not supported.' );
	...
	function setModelTransforms( FBXTree, model, modelNode ) {
		...
		if ( 'RotationOrder' in modelNode.properties ) {
			...
			if ( value > 0 && value < 6 ) {
				...
				console.warn( 'THREEFBXLoader: unsupported Euler Order: %s. Currently only XYZ order is supported. Animations and rotations may be incorrect.', enums[ value ] );

			} else if ( value === 6 ) {

				console.warn( 'THREEFBXLoader: unsupported Euler Order: Spherical XYZ. Animations and rotations may be incorrect.' );
	...
	function parseAnimStacks( FBXTree, connections, layersMap ) {
		...
		for ( var nodeID in rawStacks ) {
			...
			if ( children.length > 1 ) {
				...
				console.warn( 'THREEFBXLoader: Encountered an animation stack with multiple layers, this is currently not supported. Ignoring subsequent layers.' );
